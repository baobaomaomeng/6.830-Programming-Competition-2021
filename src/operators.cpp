...
bool cmp(uint64_t val, FilterInfo &f){
    auto constant = f.constant;
    switch (f.comparison) {
    case FilterInfo::Comparison::Equal:
        return val == constant;
...
// Run
void FilterScan::run() {
    // vis = new bool[relation_.size()];
    // int t = relation_.size()/200000;
    // if(t>8)t=8;
    // #pragma omp parallel for num_threads(t)
    // for(int i=0;i<relation_.size();++i)vis[i]=true;
    // for(auto &f:filters_){
    //     auto compare_col = relation_.columns()[f.filter_column.col_id];
    //     for(int j = 0;j<relation_.size();++j){
    //         if(!cmp(compare_col[j], f)){
    //             vis[j] = false;
    //         }
    //     }
    // }
    // result_size_ = 0;
    // #pragma omp parallel for reduction(+:result_size_) num_threads(t)
    // for(int i = 0;i<relation_.size();++i){
    //     if(vis[i])result_size_++;
    // }

    // #pragma omp paraller for num_threads(t);
    // for(int i = 0 ;i < input_data_.size();++i){
    //     tmp_results_[i].reserve(result_size_);
    //     int num = 0;
    //     for(int j = 0;j<relation_.size();++j){
    //         if(vis[j])tmp_results_[i][num++] = input_data_[i][j];
    //     }
    // }
